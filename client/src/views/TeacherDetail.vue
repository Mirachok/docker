  ...
  <div class="card">
    ...
    <template v-else>
      ...
      <template v-if="item">
        ...
        <div><b>Отдел:</b> {{ item.department }}</div>
        <div><b>Должность:</b> {{ item.position }}</div>
        <div><b>Email:</b> {{ item.email }}</div>
        <div><b>Телефон:</b> {{ item.phone }}</div>

        <div style="margin-top:12px;">
          <button class="btn" @click="$router.push('/teachers')">← Назад</button>
          <button v-if="role==='admin'" class="btn" style="margin-left:8px;"
                  @click="$router.push(`/teachers/${item.id}/edit`)">✏️ Редактировать</button>
        </div>
      </template>
    </template>
  </div>
...
async function load() {
  ...
  err.value = "";
  try { item.value = await api(`/teachers/${route.params.id}`); }
